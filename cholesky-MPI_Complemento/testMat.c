...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    // printf("my_rank:   %d, inicio: %d\n",my_rank,inicio);
    fim = calc_fim(n_bar, n, i, p, my_rank);
    // printf("my_rank:   %d,  fim: %d\n",my_rank,fim);
    // if (my_rank==1)
    // {
    //   printf("i:   %d, inicio: %d\n",i,inicio);
    //   printf("i:   %d,  fim: %d\n",i,fim);
    ...
    if (inicio <= fim)
    {
      ...
      {
        // printf("\nENVIANDO para 0\n");
        MPI_Send(&A[i][inicio], (fim - inicio + 1), MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
        // printf("\nEnviado para 0\n");
      }
      else
      {
        ...
        for (int k = 1; k < p; k++)
        {
          ...
          // printf("\nTo em 0.2\n");
          MPI_Recv(&A[i][inicio], (fim - inicio + 1), MPI_DOUBLE, k, 0, MPI_COMM_WORLD, &status);
          // print_mat(A,n,my_rank);
        }
      }
    }
